...
export const updateSelfProfileController = async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      user: {
        id: result.user.UserID,
        username: result.updatePayload.Username ?? result.user.Username,
        fullName: result.updatePayload.FullName ?? result.user.FullName,
        password: result.updatePayload.PasswordHash ? "UPDATED" : "UNCHANGED",
        role: result.user.UserRole,
        updatedAt: new Date(),
      },
    });
...
export const removeProfileController = async (req, res) => {
  ...
  res.status(200).json({
    success: true,
    message: `Profile deleted successfully for user ${user.Username}`,
    user: {
      id: user.UserID,
